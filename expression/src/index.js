...
class QueryBuilder {
    ...
    cleanTree (item) {
        ...
        item.children = item.children.map(item => { return this.cleanTree(item) }).filter(item => item);
        return item;
    }
    ...
    conjunctionExpressionConstraint (node, value, queryPart) {
        ...
        node.children.forEach((child) => {
            if (child.opData == "subExpressionConstraint") {
                computer.resolve(child, and["$and"]);
            }
        });
        queryPart.push(and);
    };
    ...
    attribute (node, ast, queryPart) {
        ...
        if (condition.targetNode) {
            if (condition.targetNode.opData == "simpleExpressionConstraint") {
                var targetExp = this.readSimpleExpressionConstraint(condition.targetNode, ast);
